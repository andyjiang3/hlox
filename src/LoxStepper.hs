...
instance PP Store where
 ...
 pp (St e es _) = case Map.lookup e es of
   ...
   Just env -> let mem = PT {table = memory env} in  LoxSyntax.pp mem


newtype PrintableTable = PT {table :: Map Name Table}


instance PP PrintableTable
pp (PT t) = undefined
...
indexRecursive (t, n) id = do
 ...
 let valMaybe = case n of 
        LName nm -> memory env !? t >>= (!? nm)
        LArrayIndex _ _ -> undefined
 case valMaybe of
   Just val -> return val
       ...
       do
        ...
        --  guard (isJust $ Map.lookup name oldTable)
         let maybeTable :: Maybe Table = case name of
               ...
               ref@(LArrayIndex lv e) -> go ref (const $ Just val)  where
